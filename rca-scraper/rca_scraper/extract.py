...
from .client import build_payload, load_capture
...
def run(cfg: Config, report_name: str, *, max_tiles: int | None = None,
    ...
    # Base payload: the captured query with mode flags + a big page size applied.
    ...
    try:
        if store.get_meta("complete"):
            log.info("Report '%s' already marked complete (%d rows). "
                     "Delete the table or checkpoint to re-run.",
                     report_name, store.count())
        else:
